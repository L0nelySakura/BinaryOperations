...
public:
    ...
    // f) Периодичность опроса наличия входного файла (сек) - всегда активно в периодическом режиме
    ...
    // Интервал запуска обработки по таймеру (сек)
...
private:
    ...
    int run_interval_sec_ = 30;          // Дефолтно 30 секунд
    int check_files_interval_sec_ = 10;  // Дефолтно 10 секунд
};
